  ...
  const setupHint =
    !envResolution.localEnvPath && envResolution.siblingEnvPaths.length === 0
      ? [
          ...
          `Run "pnpm -C ${relativeAppPath} convex:dev" once in an interactive terminal to let Convex configure the project, or copy an existing .env.local into this app.`,
        ]
      : [
          `Loaded env from the available app .env.local files, but they are still missing ${missingList}.`,
          `Run "pnpm -C ${relativeAppPath} convex:dev" interactively to finish Convex setup, then retry.`,
        ];
