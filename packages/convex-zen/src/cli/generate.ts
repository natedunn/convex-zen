...
function hasOAuthProviders(authSource: string): boolean {
  const callContent = extractDefineConvexZenCallContent(authSource);
  if (!callContent) {
    return false;
  }
...
function extractDefineConvexZenCallContent(source: string): string | null {
  ...
  return stripped.slice(openParenIndex + 1, closeParenIndex);
}

function isOAuthProxyReturnTargetRule(
  value: unknown
): value is NonNullable<OAuthProxyConfig["allowedReturnTargets"]>[number] {
  if (!value || typeof value !== "object") {
    return false;
  }
  const record = value as Record<string, unknown>;
  if (record.type === "webUrl" && typeof record.url === "string") {
    return true;
  }
  if (
    record.type === "webUrlPattern" &&
    typeof record.pattern === "string"
  ) {
    return true;
  }
  return (
    record.type === "nativeCallback" &&
...
export const handleOAuthProxyCallback = action({
  ...
  handler: async (ctx, args) => {
    ...
    return ctx.runAction(componentFn<"action">("core/gateway:handleProxyCallback"), {
      provider,
      code: args.code,
      state: args.state,
      callbackUrl: args.callbackUrl,
...
export const exchangeOAuthProxyCode = action({
  ...
  handler: async (ctx, args) => {
    return ctx.runAction(componentFn<"action">("core/gateway:exchangeProxyCode"), {
      code: args.code,
      ipAddress: args.ipAddress,
      userAgent: args.userAgent,
      checkBanned: true,
...
function renderMetaHelperFile(
  ...
  configMeta: { oauthProxy?: OAuthProxyConfig } | undefined
): string {
...
export const authMeta = ${JSON.stringify(
    {
      ...
      ...(configMeta !== undefined ? { config: configMeta } : {}),
    },
    null,
    2
  )} as const;
...
export async function generateAuthFunctions(
  ...
  const parsedOAuthProxyConfig = parseOAuthProxyConfigFromSource(authSourceContents);
  const configMeta =
    parsedOAuthProxyConfig !== undefined
      ? { oauthProxy: parsedOAuthProxyConfig }
      : undefined;
  ...
  await upsertGeneratedFile(
    {
      ...
      content: renderMetaHelperFile(coreMeta, pluginMeta, configMeta),
    },
    options,
    result
  );
